    ...
    match migrator.run(&pool).await {
        ...
        Err(sqlx::migrate::MigrateError::VersionMismatch(v)) => {
            ...
            if allow_checksum_repair {
                tracing::warn!(version = v, "migration checksum mismatch detected; repairing");
                let repaired = file_storage_backend::database::pool::repair_migration_checksum(
                    &pool, &migrator, v,
                ).await?;
                if repaired {
                    migrator.run(&pool).await.map_err(|e| anyhow::anyhow!("Failed to run migrations after checksum repair: {}", e))?;
                } else {
                    return Err(anyhow::anyhow!("Failed to repair migration checksum for version {}", v));
                }
            } else {
                let allow_reset =
                    std::env::var("RESET_DB_ON_MIGRATION_MISMATCH").as_deref() == Ok("1");
                if allow_reset {
                    tracing::warn!(version = v, "migration checksum mismatch detected; resetting public schema");
                    file_storage_backend::database::pool::reset_public_schema(&pool).await?;
                    file_storage_backend::database::pool::pre_migration_repairs(&pool).await?;
                    migrator.run(&pool).await.map_err(|e| anyhow::anyhow!("Failed to run migrations after schema reset: {}", e))?;
                } else {
