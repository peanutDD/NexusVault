...
impl FileService {
    ...
    pub async fn create_file_from_path(
        ...
        let file_id = if let Some(existing) = existing_file {
            ...
            if let Some(embedding_service) = &self.embedding_service {
                let file_clone = self.files_repo.find_by_id(existing_file_id, user_id).await?.ok_or(AppError::NotFound)?;
            ...
            // 如果配置了嵌入服务，异步提取内容并生成向量嵌入（不阻塞上传流程）
            if let Some(embedding_service) = &self.embedding_service {
                let file_clone = self.files_repo.find_by_id(file_response.id, user_id).await?.ok_or(AppError::NotFound)?;
                let embedding_service_clone = embedding_service.clone();
                let storage_clone = self.storage.clone();
                let mime_type_clone = mime_type.clone();
                let original_filename_clone = original_filename.clone();
    ...
    pub async fn generate_embedding_with_content(
        ...
        let file_data = match storage.get_file(&file.file_path).await {
            Ok(data) => data,
            Err(e) => {
                tracing::warn!("Failed to read file {} for content extraction: {}", file_id, e);
                return;
            }
        };

        // 2. 提取文本内容
        let content = match FileContentExtractor::extract_text(&file_data, mime_type, original_filename) {
            Ok(text) => {
        ...
        match embedding_service.generate_embedding(&search_text).await {
            Ok(embedding) => {
                ...
                if let Err(e) = search_service.update_file_embedding(file_id, user_id, &embedding).await {
                    tracing::warn!("Failed to update file embedding for {}: {}", file_id, e);
                } else {
                    tracing::debug!("Successfully generated embedding for file {} (with content)", file_id);
                }
            }
            Err(e) => {
