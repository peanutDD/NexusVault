...
async fn test_file_service_rename_duplicate_name() {
    ...
    let file1_id = create_test_file(&pool, user_id, "existing.txt").await;
...
async fn test_file_service_chunked_upload_happy_path() {
    ...
    // 初始化分块上传
    let (upload_id, chunk_size, total_parts) = service.init_chunked_upload(user_id, init_req).await.unwrap();
    assert_eq!(chunk_size, CHUNK_SIZE);
    assert_eq!(total_parts, 3);

    // 上传第一个分块
    let chunk1_data = Bytes::from(vec![0u8; CHUNK_SIZE as usize]);
    service.upload_chunk(upload_id, user_id, 1, chunk1_data, None).await.unwrap();

    // 上传第二个分块
    let chunk2_data = Bytes::from(vec![1u8; CHUNK_SIZE as usize]);
    service.upload_chunk(upload_id, user_id, 2, chunk2_data, None).await.unwrap();

    // 上传第三个分块（最后一个，小于CHUNK_SIZE）
    let chunk3_data = Bytes::from(vec![2u8; 100]);
    service.upload_chunk(upload_id, user_id, 3, chunk3_data, None).await.unwrap();

    // 查询上传状态
    let (uploaded_parts, parts_total) = service.chunked_upload_status(upload_id, user_id).await.unwrap();
    assert_eq!(uploaded_parts.len(), 3);
    assert_eq!(parts_total, 3);

    // 完成上传
    let complete_req = CompleteChunkedUploadRequest { folder_id: None };
    let result = service.complete_chunked_upload(upload_id, user_id, complete_req).await;
    assert!(result.is_ok());
...
async fn test_file_service_chunked_upload_invalid_part_index() {
    ...
    let (upload_id, _, total_parts) = service.init_chunked_upload(user_id, init_req).await.unwrap();
    assert_eq!(total_parts, 1);

    // 尝试上传不存在的分块
    let chunk_data = Bytes::from(vec![0u8; 100]);
    let result = service.upload_chunk(upload_id, user_id, 2, chunk_data, None).await;
    
    assert!(result.is_err());
    matches!(result.err().unwrap(), FileServiceError::InvalidChunkIndex { .. });
}
...
async fn test_file_service_chunked_upload_missing_chunks() {
    ...
    let (upload_id, _, _) = service.init_chunked_upload(user_id, init_req).await.unwrap();

    // 只上传一个分块
    let chunk_data = Bytes::from(vec![0u8; CHUNK_SIZE as usize]);
    service.upload_chunk(upload_id, user_id, 1, chunk_data, None).await.unwrap();
    ...
    assert!(result.is_err());
    matches!(result.err().unwrap(), FileServiceError::MissingUploadedChunks { .. });
}
...
async fn test_file_service_chunked_upload_duplicate_chunk() {
    ...
    // 上传同一个分块两次
    let chunk_data = Bytes::from(vec![0u8; CHUNK_SIZE as usize]);
    service.upload_chunk(upload_id, user_id, 1, chunk_data.clone(), None).await.unwrap();
    let result = service.upload_chunk(upload_id, user_id, 1, chunk_data, None).await;
...
async fn test_file_service_instant_upload_happy_path() {
    ...
    // 先创建一个文件
    let file_id = create_test_file(&pool, user_id, "original.txt").await;
...
async fn test_file_service_quota_exceeded() {
    ...
    // 尝试创建另一个文件（应该超出配额）
    let result = service.ensure_can_store_detailed(user_id, "text/plain", 1024).await;
    assert!(result.is_err());
    let err = result.err().unwrap();
