...
use common::{cleanup_test_data, create_test_pool, create_test_user, create_test_file, create_test_folder, init_test_env};
...
async fn test_files_repo_crud() {
    ...
    // 查询文件
    let file: Option<(uuid::Uuid, String)> = sqlx::query_as(
...
async fn test_folders_repo_hierarchy() {
    ...
    // 验证层级关系
    let child: Option<(uuid::Uuid, Option<uuid::Uuid>)> = sqlx::query_as(
...
async fn test_files_repo_list() {
    ...
    // 查询文件列表
    let files: Vec<(uuid::Uuid,)> = sqlx::query_as(
        "SELECT id FROM files WHERE user_id = $1 ORDER BY created_at"
    )
    .bind(user_id)
    .fetch_all(&pool)
    .await
