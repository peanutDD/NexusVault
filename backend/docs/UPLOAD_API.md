...
若服务器已存在**相同内容**（同 SHA-256 + 同大小）的文件，可直接创建一条新文件记录并复用存储，无需上传内容。
...
**限制**：每用户同时进行的分片上传（未完成/未取消）不能超过 **5 个**，否则 init 返回 **400**，文案：`同时进行的分片上传不能超过 5 个，请先完成或取消其他大文件上传`。前端应根据该限制禁用「添加大文件」或提示用户。
...
| 每用户同时分片上传数（大文件数） | 5 个 | 超过后 init 返回 400，需先 complete 或 abort 其他会话 |
...
| **大文件数** | 5 个 | 其中 ≥100MB 的视为大文件，最多 5 个（即最多 5 个大文件 + 15 个小文件） |

- 后端：分片上传 init 时校验「当前用户未过期的分片会话数」≤ 5，超过则返回 400。
- 前端：
  - 添加文件时：**先按「同名 + 同大小 + 同修改时间」去重**（与当前队列及本批内重复的只保留一份），再校验总数量 ≤ 20、大文件 ≤ 5；超限部分不加入队列。若存在重复则提示「已忽略 x 个重复文件（同名同大小）」。
  - **分开提醒**：展示两行统计——「文件数量 x/20」「大文件（≥100MB）y/5」；因总数量超限未添加时单独提示总数量；因大文件数量超限未添加时单独提示大文件数量；已达 20 或 5 时分别常驻一句说明。
...
- 单次 20 个、大文件 5 个的约定与分开提醒见本文档第 6 节。
