...
- ✅ 文件列表（虚拟列表 + 无限滚动：滚动到底自动加载，`Load more` 按钮兜底）
- ✅ 文件过滤与分组：
  - `By Type`：按类型分组（Images / GIF / Videos / Audio / PDF / Markdown / Text / Archives / Docs / Others），每组支持独立全选；分页加载时严格按类型顺序（前一类加载完再进入下一类）
  - `By Time`：按天分组（例如 `Jan 24, 2025`），文件与文件夹都会进入对应日期分组，支持分组级全选
- ✅ 文件列表交互（排序与记忆位置）：
  - 普通排序（按名称/时间/大小）对文件与文件夹统一排序并混排展示（不强制“文件夹置顶”）
  - 名称排序为自然数值排序：`1, 2, 10` 按数字顺序；`1/01/001` 作为同一数字稳定排序
  - 时间排序分页稳定：同时间戳文件按 `id` 作为二级排序，跨页不会抖动/穿插
  - 仅浏览器后退/前进（POP）时恢复滚动位置，返回上一级目录可回到离开时的浏览位置；点击进入子目录从顶部开始
- ✅ 文件搜索（按名称模糊匹配）
- ✅ 文件下载 / 批量下载（ZIP 打包）
...
- ✅ 文件预览
  - 图片 / PDF / 文本 / 音视频内联预览
  - **图片预览交互**：支持放大、缩小、旋转与重置；图片放大到 `1x` 以上后，可用鼠标/触摸/笔拖动查看被遮挡区域
  - **PDF 预览（移动端兼容）**：前端对 PDF 使用 `GET /api/files/:id/preview?token=...` 直连流式 URL 进行内联预览（避免部分移动端对 `blob:` + `iframe` 的兼容问题）
  - **GIF 预览**：GIF 默认走 HLS（.m3u8 + .ts）以支持“边转边播”；旧版 MP4 预览接口保留作为后备方案
...
- **阶段 3：前端设计系统与 UX 打磨**
  ...
  - **当前状态**：
    ...
    - **Markdown 预览功能**：支持完整的 Markdown 渲染（包括 GFM 扩展、代码高亮、HTML 图片），当前仅保留赛博暗色主题，并通过后端代理接口解决外链图片防盗链问题。
