...
当本轮修复了 Gemini Review 的 `Medium/Medium+/High/Critical` 问题时，默认追加 `<repo_root>/docs/auto-review-ledger.md`。

该 ledger 是本地问题解决记录，不替代 PR 评论：

- 记录 Gemini 原问题、严重级别、文件和行号。
- 记录 Codex 对应状态：`resolved`、`pending`、`blocked`。
- 记录解决答案或未解决原因。
- 记录本轮修改文件。
