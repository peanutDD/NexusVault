...
describe("Shares page", () => {
  it("renders the share center in bounded neuromorphic panels and exposes share events", async () => {
    ...
    expect(baseCss).toContain("linear-gradient(145deg, #7c3aed, #6d28d9)");
    ...
    expect(baseCss).toContain("--share-center-neu-raised-shadow:");
    expect(baseCss).toContain("--share-center-neu-inset-shadow:");
    expect(baseCss).toContain("--share-center-neu-pressed-shadow:");
    expect(baseCss).toContain("box-shadow: var(--share-center-neu-inset-shadow) !important;");
    expect(baseCss).toContain("box-shadow: var(--share-center-neu-raised-shadow) !important;");
    ...
    expect(screen.getByTestId("share-row-share-1")).toHaveClass(
      ...
      "shadow-[var(--settings-kpi-shadow)]",
    );
    ...
    expect(screen.getByTestId("share-status-badge-share-1")).toHaveClass(
      ...
      "w-fit",
      "items-center",
      "[background:var(--share-badge-green-bg)]",
      "text-[var(--share-badge-green-text)]",
    );
    ...
    expect(screen.getByTestId("share-status-badge-share-2")).toHaveClass(
      "share-color-badge",
      "shareCenterFlatBadge",
      "[background:var(--share-badge-yellow-bg)]",
      "text-[var(--share-badge-yellow-text)]",
    );
  ...
  it("keeps Share Center CodePen surfaces scoped by light and dark theme", () => {
    ...
    expect(baseCss).toContain("--share-center-codepen-button-default-bg: linear-gradient(145deg, #363D4C, #242C39);");
    expect(baseCss).toContain("--share-center-codepen-button-primary-bg: linear-gradient(145deg, #8235DA, #7A2ECE);");
    expect(baseCss).toContain("--share-center-codepen-button-success-bg: linear-gradient(145deg, #59B964, #51AA5A);");
  ...
  it("matches the CodePen neuromorphic depth without changing the Share Center palette", () => {
    ...
    expect(baseCss).toContain(
      "--share-center-neu-raised-bg: linear-gradient(145deg, var(--codepen-neu-bg), var(--codepen-neu-bg-secondary));",
    );
    expect(baseCss).toContain(
      "--share-center-neu-inset-bg: linear-gradient(145deg, var(--codepen-neu-bg-secondary), var(--codepen-neu-bg));",
    );
    expect(baseCss).toContain(
      "--share-center-neu-pressed-bg: linear-gradient(145deg, var(--codepen-neu-bg-secondary), var(--codepen-neu-bg));",
    );
    expect(baseCss).toMatch(
      /\.shareCenterCodepenShell,\s*\.shareCenterNeuRaisedPanel\s*\{[^}]*background: var\(--share-center-neu-raised-bg\) !important;/,
    );
    expect(baseCss).toMatch(
      /\.shareCenterNeuRaisedSurface\s*\{[^}]*background: var\(--share-center-neu-raised-bg\) !important;[^}]*box-shadow: var\(--share-center-neu-raised-soft-shadow\) !important;/,
    );
    expect(baseCss).toMatch(
      /\.shareCenterCodepenFrame,\s*\.shareCenterCodepenList\s*\{[^}]*background: var\(--share-center-neu-inset-bg\) !important;[^}]*box-shadow: var\(--share-center-neu-inset-shadow\) !important;/,
    );
    expect(baseCss).toMatch(
      /\.shareCenterNeuInsetSurface,\s*\.shareCenterNeuDataList,\s*\.shareCenterNeuControl\s*\{[^}]*background: var\(--share-center-neu-inset-bg\) !important;/,
    );
    expect(baseCss).toMatch(
      /\.shareCenterNeuPressedSurface\s*\{[^}]*background: var\(--share-center-neu-pressed-bg\) !important;[^}]*box-shadow: var\(--share-center-neu-pressed-shadow\) !important;/,
    );
    expect(baseCss).toContain("--codepen-neu-bg: #1F2937;");
    expect(baseCss).toContain("--codepen-neu-bg: #E0E5EC;");
    expect(baseCss).toContain("--codepen-neu-bg-secondary: #374151;");
    expect(baseCss).toContain("--codepen-neu-bg-secondary: #D1D9E6;");
    expect(baseCss).not.toContain("--share-center-neu-raised-bg: linear-gradient(145deg, #");
    expect(baseCss).not.toContain("--share-center-neu-inset-bg: linear-gradient(145deg, #");
    expect(baseCss).not.toContain("--share-center-neu-pressed-bg: linear-gradient(145deg, #");
  });
  ...
  it("styles inbox review actions as colored status pills", async () => {
    ...
    const shareGreenBackgroundTokens = [
      ...tokenCss.matchAll(/--share-badge-green-bg:\s*([^;]+);/g),
    ].map((match) => match[1].trim());
    const shareYellowBackgroundTokens = [
      ...tokenCss.matchAll(/--share-badge-yellow-bg:\s*([^;]+);/g),
    ].map((match) => match[1].trim());
    const shareRedBackgroundTokens = [
      ...tokenCss.matchAll(/--share-badge-red-bg:\s*([^;]+);/g),
    ].map((match) => match[1].trim());
    ...
    expect(basePillCss).toContain("box-shadow: var(--share-center-neu-raised-soft-shadow) !important;");
    ...
    expect(shareGreenBackgroundTokens).toEqual(["#10B981", "#10B981", "#10B981", "#10B981"]);
    expect(shareYellowBackgroundTokens).toEqual(["#F59E0B", "#F59E0B", "#F59E0B", "#F59E0B"]);
    expect(shareRedBackgroundTokens).toEqual(["#FCA5A5", "#FCA5A5", "#FCA5A5", "#FCA5A5"]);
    ...
    expect(disabledPillCss).toContain("box-shadow: var(--share-center-neu-pressed-shadow) !important;");
  ...
  it("renders File Request review submissions as a fluid neuromorphic inspector", async () => {
    ...
    expect(compactBaseCss).toMatch(/\.fileRequestInboxDetailPanel \{[^}]*box-shadow: var\(--share-center-neu-raised-shadow\) !important;/);
    expect(compactBaseCss).toMatch(/\.fileRequestInboxSubmissionCard \{[^}]*box-shadow: var\(--share-center-neu-raised-soft-shadow\) !important;/);
    expect(compactBaseCss).toMatch(/\.fileRequestInboxSummaryStrip \{[^}]*box-shadow: var\(--share-center-neu-inset-shadow\) !important;/);
    expect(compactBaseCss).toMatch(/\.fileRequestInboxFileItem \{[^}]*box-shadow: var\(--share-center-neu-raised-soft-shadow\) !important;/);
    expect(compactBaseCss).toMatch(/\.fileRequestInboxFlat(Input|Field) \{[^}]*box-shadow: var\(--share-center-neu-inset-shadow\) !important;/);
  ...
  it("keeps share and file request management usable on narrow mobile screens", async () => {
    ...
    expect(baseCss).toContain("background: var(--codepen-neu-bg) !important;");
    expect(baseCss).toContain("--codepen-neu-bg-secondary: #D1D9E6;");
    expect(baseCss).toContain("--share-center-neu-form-input-bg:");
    expect(baseCss).toContain("--share-center-neu-form-input-shadow:");
    expect(baseCss).toMatch(
      /\.shareCenterNeuFormInput \{[^}]*background: var\(--share-center-neu-form-input-bg\) !important;[^}]*box-shadow: var\(--share-center-neu-form-input-shadow\) !important;/,
    );
    expect(filtersCss).toMatch(
      /\.neuromorphic-style \.filtersSearchPill,[^}]*background: var\(--neu-inset-bg\);[^}]*box-shadow: var\(--neu-inset-shadow\);/,
    );
    expect(baseCss).toMatch(
