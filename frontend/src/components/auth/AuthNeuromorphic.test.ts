...
describe("auth Neuromorphic style contract", () => {
  ...
  it("maps primary auth buttons to raised and pressed Neuromorphic states without scaling", () => {
    expect(AUTH_BUTTON_CLASSES).toContain("neu-semantic-raised");
  ...
  it("removes obsolete Shape Wave, aura, edge, and glow assets from auth", () => {
    const tokens = readFileSync("src/styles/tokens.css", "utf8");

    expect(tokens).not.toContain("--auth-shape-wave");
    ...
    expect(existsSync("src/components/auth/AuthShapeWaveBackground.tsx")).toBe(false);
  });
});
