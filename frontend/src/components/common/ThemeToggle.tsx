...
export default function ThemeToggle({ className, showLabel = false }: ThemeToggleProps) {
  ...
  const nextThemeLabel = effectiveTheme === 'dark' ? '浅色' : effectiveTheme === 'light' ? '紫色' : '深色';
  const currentThemeLabel = effectiveTheme === 'dark' ? '深色' : effectiveTheme === 'light' ? '浅色' : '紫色';
    ...
    >
      <div className={cn('flex items-center', showLabel ? 'gap-2' : 'gap-0')}>
        {effectiveTheme === 'light' ? (
          <Sun className="h-5 w-5 transition-transform duration-200 hover:rotate-12" aria-hidden="true" />
        ) : effectiveTheme === 'dark' ? (
          <Moon className="h-5 w-5 transition-transform duration-200 hover:rotate-12" aria-hidden="true" />
        ) : (
          <Sparkles className="h-5 w-5 transition-transform duration-200 hover:rotate-12" aria-hidden="true" />
        )}
        {showLabel && (
