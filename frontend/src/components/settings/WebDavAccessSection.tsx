...
const diagnosticsHeaderActionClass =
  "settings-neu-inset-control inline-flex min-h-[clamp(2.5rem,5.8vw,2.75rem)] w-full items-center justify-center gap-[clamp(0.2925rem,0.7vw,0.375rem)] rounded-[clamp(0.85rem,2vw,1rem)] border border-[var(--settings-panel-border)] [background:var(--settings-panel-bg)] px-[clamp(0.68rem,1.55vw,0.85rem)] py-[clamp(0.5rem,1.1vw,0.625rem)] text-[length:var(--settings-text-sm)] font-semibold text-[var(--settings-chip-text)] shadow-[var(--settings-panel-shadow)] transition-[border-color,background,filter,opacity] duration-200 hover:border-[var(--settings-panel-border-hover)] hover:[filter:brightness(1.04)] active:[filter:brightness(0.98)] sm:w-auto";

const diagnosticsPanelClass =
  "settings-neu-inset-panel rounded-[clamp(1rem,2.4vw,1.25rem)] border border-transparent [background:var(--neu-inset-bg)] p-[clamp(1rem,2.25vw,1.25rem)] shadow-[var(--neu-inset-shadow)] transition-[box-shadow,background,filter] duration-200";

const diagnosticsFieldCardClass =
  "rounded-[clamp(0.68rem,1.55vw,0.8rem)] border border-[var(--settings-panel-border)] [background:var(--settings-form-input-bg)] p-[clamp(0.68rem,1.55vw,0.85rem)] shadow-[var(--settings-form-input-shadow)]";
...
const diagnosticsMetricChipClass =
  "inline-flex w-fit items-center rounded-full border border-[var(--settings-kpi-border)] [background:var(--settings-kpi-bg)] px-[clamp(0.48rem,1.05vw,0.62rem)] py-[clamp(0.195rem,0.45vw,0.25rem)] text-[length:var(--settings-text-xs)] font-semibold text-[var(--settings-panel-value)] shadow-[var(--settings-kpi-shadow)]";
...
export default function WebDavAccessSection() {
  ...
  const { data: activity = [], isLoading: activityLoading } = useWebDavActivity();
  const {
    data: diagnostics = [],
  ...
  const handleCreateWizardToken = useCallback(() => {
    ...
    createWizardToken.mutate({
      ...
      onError: (error) => {
        setWizardError(getErrorMessage(error, "Failed to create WebDAV token"));
      },
    });
  }, [createWizardToken, runConnectionTest, wizardReadOnly, wizardRootFolderId, wizardTokenName]);
      ...
      >
        <section
          ...
          className={settingsPanelClass(
            "overflow-hidden [background:var(--settings-kpi-bg)] p-[clamp(1rem,2.25vw,1.25rem)] shadow-[var(--settings-kpi-shadow)]",
          )}
        >
          <div
          ...
          <div className="mt-[clamp(1rem,2.25vw,1.25rem)] rounded-[clamp(0.7rem,1.6vw,0.875rem)] border border-[var(--settings-panel-border)] [background:var(--settings-form-input-bg)] p-[clamp(0.78rem,1.8vw,1rem)] shadow-[var(--settings-form-input-shadow)]">
              ...
              <button
                ...
                className={settingsPrimaryPillClass(
                  `font-brand w-full min-w-[clamp(8.75rem,12vw,9.5rem)] gap-[clamp(0.39rem,0.9vw,0.5rem)] md:w-auto ${
                    copyUrlPressed
                      ? "webdav-copy-url-pressed scale-[0.98] translate-y-[clamp(0.04rem,0.1vw,0.0625rem)] [background:var(--settings-action-bg-hover)] shadow-[var(--settings-action-shadow-active)]"
                      : ""
                  }`,
                )}
          ...
          <div className="mt-[clamp(0.78rem,1.8vw,1rem)] grid gap-[clamp(0.585rem,1.35vw,0.75rem)] sm:grid-cols-2">
            <div className="rounded-[clamp(0.7rem,1.6vw,0.75rem)] border border-[var(--settings-kpi-border)] [background:var(--settings-kpi-bg)] p-[clamp(0.78rem,1.8vw,1rem)] shadow-[var(--settings-kpi-shadow)]">
        ...
        >
          ...
          >
            <div className="min-w-0">
              ...
              <p className="mt-[clamp(0.39rem,0.9vw,0.5rem)] text-[length:var(--settings-text-xs)] leading-relaxed text-[var(--settings-subtitle)]">
                Creates a WebDAV-enabled read/write token that expires in 90 days, then tests PROPFIND while the token is visible.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mt-[clamp(0.78rem,1.8vw,1rem)] grid gap-[clamp(0.78rem,1.8vw,1rem)] lg:grid-cols-[minmax(0,1fr)_minmax(0,0.8fr)]">
            <div>
              ...
              <input
                ...
                className="settings-neu-inset-control mt-[clamp(0.39rem,0.9vw,0.5rem)] w-full rounded-[clamp(0.7rem,1.6vw,0.75rem)] border border-[var(--settings-form-input-border)] [background:var(--settings-form-input-bg)] px-[clamp(0.78rem,1.8vw,1rem)] py-[clamp(0.68rem,1.5vw,0.75rem)] text-[length:var(--settings-text-sm)] text-[var(--settings-form-input-text)] shadow-[var(--settings-form-input-shadow)]"
              />
            </div>
            <div>
              <label
            ...
            <label className="group relative grid cursor-pointer grid-cols-[clamp(2.1rem,4vw,2.4rem)_minmax(0,1fr)] gap-[clamp(0.585rem,1.35vw,0.75rem)] rounded-[clamp(0.7rem,1.6vw,0.875rem)] border border-[var(--settings-panel-border)] [background:var(--settings-panel-bg)] px-[clamp(0.78rem,1.8vw,1rem)] py-[clamp(0.68rem,1.5vw,0.85rem)] text-[var(--settings-panel-value)] shadow-[var(--settings-panel-shadow)] lg:col-span-2">
              ...
              <span
                aria-hidden="true"
                className="settings-color-checkbox-indicator mt-[clamp(0.1rem,0.25vw,0.15rem)] flex h-[clamp(1.7rem,3.2vw,1.9rem)] w-[clamp(1.7rem,3.2vw,1.9rem)] rounded-[clamp(0.55rem,1.2vw,0.65rem)] border border-[var(--settings-chip-border)] [background:var(--settings-form-input-bg)] shadow-[var(--settings-form-input-shadow)] peer-checked:border-[var(--settings-secondary-border-hover)] peer-checked:[background:var(--settings-action-bg)]"
              />
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-[clamp(0.5rem,1.1vw,0.625rem)]">
                    ...
                    <button
                      ...
                      className="inline-flex shrink-0 items-center justify-center rounded-[clamp(0.4rem,1vw,0.5rem)] p-[clamp(0.3rem,0.8vw,0.375rem)] text-[var(--settings-subtitle)] transition-colors hover:bg-[rgba(var(--rgb-white),0.06)] hover:text-[var(--settings-panel-value)]"
                  ...
                  <p className="mt-[clamp(0.39rem,0.9vw,0.5rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)]">
                    Read/write · expires {createdToken.expires_at ? new Date(createdToken.expires_at).toLocaleDateString() : "never"}
                  </p>
                </div>
                <button
                  type="button"
          ...
          {(wizardMessage || wizardError || testingConnection) && (
            <ErrorMessage
              ...
              autoDismissMs={testingConnection ? undefined : wizardError ? 5000 : 3000}
          ...
          >
            ...
            <ol className="mt-[clamp(0.78rem,1.8vw,1rem)] space-y-[clamp(0.585rem,1.35vw,0.75rem)]">
              {setupSteps.map((step, index) => (
                ...
                >
                  <span className="font-brand flex h-[clamp(1.5rem,3vw,1.75rem)] w-[clamp(1.5rem,3vw,1.75rem)] items-center justify-center rounded-full border border-[var(--settings-chip-border)] [background:var(--settings-chip-bg)] text-[length:var(--settings-text-xs)] font-semibold text-[var(--settings-chip-text)] shadow-[var(--settings-secondary-shadow)]">
                    {index + 1}
                  </span>
                  <span>{step}</span>
                </li>
          ...
          >
            ...
            <div className="mt-[clamp(0.78rem,1.8vw,1rem)] grid gap-[clamp(0.39rem,0.9vw,0.5rem)] sm:grid-cols-2 lg:grid-cols-1">
              {clientNotes.map(([name, detail]) => (
                <div
                  key={name}
                  className="grid gap-[clamp(0.195rem,0.45vw,0.25rem)] rounded-[clamp(0.7rem,1.6vw,0.75rem)] border border-[var(--settings-kpi-border)] [background:var(--settings-kpi-bg)] p-[clamp(0.585rem,1.35vw,0.75rem)] shadow-[var(--settings-kpi-shadow)]"
                >
        ...
        <section
          ...
          className={diagnosticsPanelClass}
        >
          <div className="flex flex-col gap-[clamp(0.78rem,1.8vw,1rem)] md:flex-row md:items-center md:justify-between">
          ...
          {diagnosticsLoading ? (
            ...
            <div
              ...
              className="mt-[clamp(0.78rem,1.8vw,1rem)] grid max-h-[var(--webdav-diagnostics-list-max)] gap-[var(--webdav-diagnostics-list-gap)] overflow-y-auto pr-[clamp(0.2925rem,0.7vw,0.375rem)]"
            ...
            >
              {diagnostics.map((item) => {
                ...
                const accessMode = item.webdav_read_only ? "Read-only" : "Read/write";
                const rootScope = item.webdav_root_folder_id ?? "Full account";
                return (
                  <article
                    key={item.token_id}
                    data-testid="webdav-diagnostics-item"
                    className="min-h-[var(--webdav-diagnostics-row-height)] rounded-[clamp(0.7rem,1.6vw,0.75rem)] border border-[var(--settings-panel-border)] [background:var(--settings-kpi-bg)] p-[clamp(0.78rem,1.8vw,1rem)] shadow-[var(--settings-kpi-shadow)]"
                  >
                    <div className="flex flex-col gap-[clamp(0.585rem,1.35vw,0.75rem)] sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <span className="w-fit rounded-full border border-[var(--settings-chip-border)] [background:var(--settings-chip-bg)] px-[clamp(0.585rem,1.35vw,0.75rem)] py-[clamp(0.2925rem,0.7vw,0.375rem)] text-[length:var(--settings-text-xs)] font-semibold text-[var(--settings-chip-text)]">
                        {accessMode}
                      </span>
                    </div>
                    <div className="mt-[clamp(0.68rem,1.55vw,0.85rem)] grid gap-[clamp(0.58rem,1.35vw,0.75rem)] md:grid-cols-2">
                      <section className={diagnosticsFieldCardClass}>
                        <p className={diagnosticsFieldLabelClass}>Latest source</p>
                        <p className={`${diagnosticsFieldValueClass} break-all`}>
                        ...
                        <p className={diagnosticsFieldDescriptionClass}>
                          Most recent IP and client fingerprint seen on this token.
                        </p>
                      </section>

                      <section className={diagnosticsFieldCardClass}>
                        <p className={diagnosticsFieldLabelClass}>Access mode</p>
                        <p className={diagnosticsFieldValueClass}>{accessMode}</p>
                        <p className={diagnosticsFieldDescriptionClass}>
                          Permission profile currently assigned to this device token.
                        </p>
                      </section>

                      <section className={diagnosticsFieldCardClass}>
                        <p className={diagnosticsFieldLabelClass}>Request counts</p>
                        <div className="mt-[clamp(0.35rem,0.8vw,0.45rem)] flex flex-wrap gap-[clamp(0.35rem,0.8vw,0.45rem)]">
                          <span className={diagnosticsMetricChipClass}>Reads {item.read_count}</span>
                          <span className={diagnosticsMetricChipClass}>Writes {item.write_count}</span>
                        </div>
                        <p className={diagnosticsFieldDescriptionClass}>
                          Read traffic versus write-capable operations observed for this token.
                        </p>
                      </section>

                      <section className={diagnosticsFieldCardClass}>
                        <p className={diagnosticsFieldLabelClass}>HTTP results</p>
                        <div className="mt-[clamp(0.35rem,0.8vw,0.45rem)] flex flex-wrap gap-[clamp(0.35rem,0.8vw,0.45rem)]">
                          {statusBuckets.map(([bucket, count]) => (
                            ...
                            >
                              ...
                              <span
                                aria-hidden="true"
                                className="mx-[clamp(0.16rem,0.42vw,0.22rem)] text-[var(--settings-panel-muted)]"
                              >
                                -
                              </span>
                        ...
                        <p className={diagnosticsFieldDescriptionClass}>
                          Response buckets grouped by outcome so client issues surface quickly.
                        </p>
                      </section>

                      <section className={`${diagnosticsFieldCardClass} md:col-span-2`}>
                        <p className={diagnosticsFieldLabelClass}>Root scope</p>
                        <p className={`${diagnosticsFieldValueClass} break-all`}>{rootScope}</p>
                        <p className={diagnosticsFieldDescriptionClass}>
                          Folder boundary this token can see when clients mount the WebDAV endpoint.
                        </p>
                      </section>
                    </div>
                  </article>
        ...
        {showActivity && (
          <section
            ...
            className="settings-neu-raised-card rounded-[clamp(1rem,2.4vw,1.25rem)] [background:var(--settings-surface-bg)] p-[clamp(1rem,2.25vw,1.25rem)] shadow-[var(--settings-surface-shadow)]"
          >
            <div
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-[clamp(0.195rem,0.45vw,0.25rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)]">
                  Audit WebDAV client requests, paths, status codes, and token names.
                </p>
              </div>
              <div
              ...
              >
                <span className="rounded-full border border-[var(--settings-chip-border)] [background:var(--settings-chip-bg)] px-[clamp(0.45rem,1vw,0.625rem)] py-[clamp(0.18rem,0.45vw,0.25rem)] text-[length:var(--settings-text-xs)] font-semibold text-[var(--settings-chip-text)] shadow-[var(--settings-secondary-shadow)]">
                  {activityLoading ? "Loading" : `${activity.length} total`}
                </span>
                <button
                  type="button"
                  aria-label="Hide recent WebDAV activity"
                  className="inline-flex items-center justify-center rounded-[clamp(0.4rem,1vw,0.5rem)] p-[clamp(0.3rem,0.8vw,0.375rem)] text-[var(--settings-subtitle)] transition-colors hover:bg-[rgba(var(--rgb-white),0.06)] hover:text-[var(--settings-panel-value)]"
            ...
            {activity.length === 0 ? (
              ...
              <div
                ...
                className="settings-neu-inset-panel mt-[clamp(0.68rem,1.6vw,1rem)] grid max-h-[clamp(16rem,42dvh,24rem)] gap-[clamp(0.3rem,0.75vw,0.5rem)] overflow-y-auto rounded-[clamp(0.7rem,1.6vw,0.75rem)] border border-[var(--settings-panel-border)] [background:var(--settings-panel-bg)] p-[clamp(0.35rem,0.9vw,0.6rem)] shadow-[var(--settings-panel-shadow)] [scrollbar-gutter:stable]"
              >
                {activity.map((event) => (
                  <div
                    key={event.id}
                    data-testid={`webdav-activity-row-${event.id}`}
                    className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-x-[clamp(0.55rem,1.35vw,1rem)] gap-y-[clamp(0.28rem,0.7vw,0.42rem)] rounded-[clamp(0.65rem,1.5vw,0.75rem)] border border-[var(--settings-kpi-border)] [background:var(--settings-kpi-bg)] p-[clamp(0.45rem,1vw,0.64rem)] text-[length:var(--settings-text-xs)] shadow-[var(--settings-kpi-shadow)] sm:grid-cols-[minmax(0,0.36fr)_minmax(0,1fr)_auto] sm:items-start sm:p-[clamp(0.58rem,1.35vw,0.75rem)]"
                  >
                    <div
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="rounded-full border border-[var(--settings-chip-border)] px-[clamp(0.4rem,0.9vw,0.5rem)] py-[clamp(0.1rem,0.3vw,0.125rem)] font-mono text-[var(--settings-subtitle)]">
                          {event.status_code}
                        </span>
                      </div>
                    </div>
