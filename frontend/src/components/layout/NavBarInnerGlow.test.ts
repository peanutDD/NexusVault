...
describe("NavBar Neuromorphic primitive contract", () => {
  ...
  it("uses one raised navigation surface and one inset action panel", () => {
    ...
    expect(panel).toContain("background: var(--neu-inset-bg)");
    expect(panel).toContain("background-image: none");
    expect(panel).toContain("box-shadow: var(--neu-inset-shadow)");
  });
