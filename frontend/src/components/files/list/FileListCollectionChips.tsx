...
import type { FileCollectionCounts, FileCollectionCountsQuery } from "../../../types/files";
...
const COLLECTIONS = [
  ...
  ["duplicates", "重复", "重复文件：仅统计已生成 content_sha256 且哈希相同的文件"],
...
export default function FileListCollectionChips({
  ...
  const [layout, setLayout] = useState({ hasOverflow: false, visibleCount: Number.POSITIVE_INFINITY });
  ...
  const collectionChipClass = (active: boolean) =>
    cn(
      "glass-chip fileListCollectionChip fileListCollectionChipCompact inline-flex min-h-[clamp(1.38rem,2.75vw,1.65rem)] min-w-0 max-w-[min(100%,clamp(4.1rem,22vw,7.4rem))] shrink-0 items-center justify-center gap-[clamp(0.18rem,0.5vw,0.28rem)] rounded-full px-[clamp(0.45rem,1.05vw,0.62rem)] py-[clamp(0.12rem,0.35vw,0.2rem)] text-center text-[clamp(0.58rem,1.25vw,0.68rem)] leading-tight [overflow-wrap:anywhere]",
      active
        ? "fileListCollectionChipActive border-[var(--cta-primary-border)] bg-[var(--btn-primary-bg)] text-[var(--btn-primary-text)] shadow-[var(--filelist-shadow-panel-soft)]"
  ...
  const renderLabel = (label: string, count?: number, extra?: string) => (
    <>
      ...
      {extra && <span className="fileListCollectionChipHint shrink-0">{extra}</span>}
      {count !== undefined && (
        <span className="fileListCollectionChipCount shrink-0" aria-hidden="true">
          {count}
        </span>
      )}
    </>
  ...
  const renderChip = (
    ...
    if (item.kind === "reset") {
      return (
        <button
          ...
          className={cn(collectionChipClass(false), "fileListCollectionResetButton")}
          aria-label="重置筛选"
          title={item.title}
        >
    ...
    if (item.kind === "all") {
      const isActive = (activeCollections.length === 0 && !activeTagId) || pendingActiveChipKey === item.key;
      return (
        <button
          key={item.key}
          ref={ref}
          type="button"
          onPointerDown={() => markPendingActive(item.key)}
          onClick={resetFilters}
    ...
    if (item.kind === "collection") {
      const isActive = activeCollections.includes(item.value) || pendingActiveChipKey === item.key;
      return (
        <button
          key={item.key}
          ref={ref}
        ...
        >
          {renderLabel(item.label, item.count, item.value === "large" ? "100MB+" : undefined)}
        </button>
      );
    }
    ...
    return (
      <button
        ...
        type="button"
        onPointerDown={() => markPendingActive(item.key)}
        onClick={() => onTagChange?.(item.tag.id)}
        aria-label={item.label}
        title={countTitle(item.title, item.count)}
        className={cn(
          collectionChipClass(activeTagId === item.tag.id || pendingActiveChipKey === item.key),
          "gap-[clamp(0.18rem,0.55vw,0.28rem)]",
        )}
      >
        <span
          className="fileListCollectionTagDot h-[clamp(0.42rem,0.9vw,0.52rem)] w-[clamp(0.42rem,0.9vw,0.52rem)] shrink-0 rounded-full"
