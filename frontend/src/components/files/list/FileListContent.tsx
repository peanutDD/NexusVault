...
const FileListContent: React.FC<FileListContentProps> = ({
  ...
  const shouldLiftPinnedStandaloneFiles =
    (isPlainSort || isGroupByTime) && !isPinnedCollectionActive(activeCollection);
  ...
  const toggleFavorite = useCallback(async (file: FileMetadata) => {
    ...
    try {
      ...
      await Promise.all([
        ...
        appQueryClient.invalidateQueries({ queryKey: FILE_COLLECTION_COUNTS_QUERY_KEY }),
      ]);
    }
  }, []);
  ...
  const hasActiveListFilter =
    activeSearch.length > 0 || Boolean(activeCollection) || Boolean(activeTagId);
      ...
      {showSearchStatus && searchMetadata && (
        ...
        >
          <div className="fileListSearchStatusShell">
            ...
            <div
              className="fileListSearchStatusRail"
            ...
            >
              ...
              {["Name", "Text", "OCR", "Tag"].map((label) => (
                <span
                  key={label}
                  className="fileListSearchStatusChip"
                >
                  {label}
                </span>
              ))}
