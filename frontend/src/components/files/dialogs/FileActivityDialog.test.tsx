...
describe("FileActivityDialog", () => {
  ...
  it("drives file activity row colors from dark and light theme palettes", () => {
    const css = readFileSync("src/styles/confirm-dialog.css", "utf8").replace(
      /\s+/g,
      " ",
    );
    ...
    expect(css).toMatch(
      /:root,\s*:root\[data-theme="dark"\],\s*:root\.dark\s*\{[^}]*--file-activity-title-bg:/,
    );
    expect(css).toMatch(
      /:root\[data-theme="light"\],\s*:root\.light\s*\{[^}]*--file-activity-title-bg:/,
    );
    expect(css).toContain("var(--file-activity-row-glow-start)");
    expect(css).toContain("var(--file-activity-title-bg)");
    expect(css).toContain("var(--file-activity-filename-text)");
    expect(css).toContain("var(--file-activity-time-text)");
    expect(css).not.toMatch(
      /\[data-theme="(?:purple|terminal|portfolio|neuromorphic)"\]/,
    );
  });

  it("matches the CodePen Alert Messages success treatment", () => {
    const css = readFileSync("src/styles/confirm-dialog.css", "utf8").replace(
      /\s+/g,
      " ",
    );

    expect(css).toContain("--file-activity-alert-bg: rgba(20, 83, 45, 0.3);");
    expect(css).toContain("--file-activity-alert-title-text: rgb(187, 247, 208);");
    expect(css).toContain("--file-activity-alert-body-text: rgb(74, 222, 128);");
    expect(css).toContain("--file-activity-alert-icon-text: rgb(74, 222, 128);");
    expect(css).toContain("--file-activity-title-bg: transparent;");
