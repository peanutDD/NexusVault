...
describe("MobilePdfPreview neuromorphic controls", () => {
  it("uses gradient-capable backgrounds and raised shadows for PDF toolbar buttons", () => {
    const source = readFileSync(resolve(__dirname, "MobilePdfPreview.tsx"), "utf8");

    expect(source).toContain("mobilePdfToolbar");
    expect(source).toContain("[background:var(--preview-pdf-toolbar-bg)]");
    expect(source).toContain("mobilePdfToolbarButton");
    expect(source).toContain("[background:var(--preview-pdf-btn-bg)]");
    expect(source).toContain("shadow-[var(--neu-control-shadow)]");
    expect(source).toContain("active:[background:var(--preview-pdf-btn-active-bg)]");
