...
4. **可操作反馈**：指出问题时尽量带「建议改法」或文档引用，便于作者直接改（见 2.5）。
...
以下与 [.cursor/skills/backend-tech-stack/SKILL.md](.cursor/skills/backend-tech-stack/SKILL.md)、[backend/ENGINEERING_PLAYBOOK.md](backend/ENGINEERING_PLAYBOOK.md)、[backend/BACKEND_TECH_STACK_AUDIT.md](backend/BACKEND_TECH_STACK_AUDIT.md) 对齐。
...
| 需要 ref 时直接作为 prop 传递，不滥用 `forwardRef` | **必须** | React 19 推荐。 |
| Context 使用 `use` 或 `<Context value>` 写法 | **建议** | React 19 模式。 |
| 表单优先使用 Action / `useActionState` / `useFormStatus`，复杂校验用 react-hook-form + Zod | **建议** | 见 React rule。 |
| 避免不必要的 `useCallback`/`useMemo`（若已开 React Compiler 可交给编译器） | **建议** | 见 React rule。 |

### 5.2 类型（TypeScript）

| 检查项 | 必须/建议 | 说明 |
|--------|-----------|------|
| 对象用 interface，联合/映射用 type；命名 PascalCase/camelCase | **必须** | 详见 `.cursor/rules/TypeScript.mdc`。 |
| 禁止 `any`，未知类型用 `unknown` | **必须** | 见 TypeScript rule。 |
| 严格 TS 配置，无隐式 any | **必须** | 与 `tsconfig` 一致。 |

### 5.3 状态（Zustand）

| 检查项 | 必须/建议 | 说明 |
|--------|-----------|------|
| 使用 selectors 细粒度订阅，避免整 store 导致无关重渲染 | **必须** | 详见 `.cursor/rules/Zustand.mdc`。 |
| 复杂客户端状态用 Zustand，避免 Context 嵌套过深 | **建议** | 见 Zustand rule。 |

### 5.4 UI（shadcn / 可访问性）

| 检查项 | 必须/建议 | 说明 |
|--------|-----------|------|
| 新增 shadcn 组件通过 CLI 安装，统一放在 `components/ui` | **必须** | 详见 `.cursor/rules/shadcn.mdc`。 |
| 类名合并使用 `cn()`（clsx + tailwind-merge） | **必须** | 见 shadcn rule。 |
| 不移除 Radix 的 `aria-*` 与键盘导航 | **必须** | 见 shadcn rule。 |
| 图标优先使用 `lucide-react` | **建议** | 见 shadcn rule。 |

### 5.5 样式（Tailwind）

| 检查项 | 必须/建议 | 说明 |
|--------|-----------|------|
| 移动优先：默认移动端，再用 `sm:`/`md:`/`lg:` 覆盖 | **必须** | 详见 `.cursor/rules/Tailwind.mdc`。 |
| 动态类名用 `clsx`/`tailwind-merge` | **必须** | 见 Tailwind rule。 |
| 避免任意值（如 `w-[123px]`），优先设计 token | **建议** | 见 Tailwind rule。 |
| 间距用 `gap`/`space-x`/`space-y` | **建议** | 见 Tailwind rule。 |
...
- [ ] 组件：函数式、ref 用法、Context/Action 用法。
- [ ] 类型：interface/type、无 any、严格 TS。
- [ ] 状态：Zustand 选择器、无 Context 滥用。
- [ ] UI：shadcn 在 `components/ui`、`cn()`、a11y。
- [ ] 样式：移动优先、clsx/merge、间距/gap。
...
| 文档 | 用途 |
|------|------|
| [backend/ENGINEERING_PLAYBOOK.md](backend/ENGINEERING_PLAYBOOK.md) | 后端性能、背压、流式、限流、DB 超时、孤儿清理等——审查「性能与稳定性」时直接引用其章节与代码位置。 |
| [backend/BACKEND_TECH_STACK_AUDIT.md](backend/BACKEND_TECH_STACK_AUDIT.md) | 后端合规基准，审查项与其中「状态」列一致。 |
