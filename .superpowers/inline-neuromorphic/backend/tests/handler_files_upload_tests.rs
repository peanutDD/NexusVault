...
#[tokio::test]
#[serial(upload_handler_db)]
async fn test_upload_file_handler_infers_media_mime_from_signature_without_extension() {
    init_test_env();
    let pool = create_test_pool().await;
    cleanup_test_data(&pool).await;

    let (_user_id, token) = login_and_get_token(&pool, "upload_signature_mime").await;
    let app = build_test_app(&pool).await;
    ...
    body.extend_from_slice(
        format!(
            "--{boundary}\r\nContent-Disposition: form-data; name=\"file\"; filename=\"HGW_dNDasAEb6N9\"\r\nContent-Type: application/octet-stream\r\n\r\n"
        )
        .as_bytes(),
    );
    body.extend_from_slice(&[
    ...
    let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["file"]["mime_type"], "image/jpeg");
    assert_eq!(json["file"]["original_filename"], "HGW_dNDasAEb6N9.jpg");
}
...
#[tokio::test]
#[serial(upload_handler_db)]
async fn test_upload_file_handler_replaces_same_name_when_previous_storage_object_is_missing() {
    init_test_env();
    let pool = create_test_pool().await;
    cleanup_test_data(&pool).await;

    let (user_id, token) = login_and_get_token(&pool, "upload_missing_storage_replace").await;
    let app = build_test_app(&pool).await;
    let (auth_name, auth_value) = bearer_auth_header(&token);

    let boundary = "test-boundary";
    let first_body = format!(
        "--{boundary}\r\nContent-Disposition: form-data; name=\"file\"; filename=\"repeat.jpg\"\r\nContent-Type: image/jpeg\r\n\r\nfirst-image\r\n--{boundary}--\r\n"
    );
    ...
    let (file_id, stale_path): (Uuid, String) = sqlx::query_as(
        "SELECT id, file_path FROM files WHERE user_id = $1 AND original_filename = 'repeat.jpg' AND deleted_at IS NULL",
    )
    .bind(user_id)
    .fetch_one(&pool)
    .await
    ...
    let row_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM files WHERE user_id = $1 AND original_filename = 'repeat.jpg' AND deleted_at IS NULL",
    )
    .bind(user_id)
    .fetch_one(&pool)
    .await
    ...
    let current_path: String = sqlx::query_scalar("SELECT file_path FROM files WHERE id = $1")
        .bind(file_id)
        .fetch_one(&pool)
        .await
        .unwrap();
    let current_bytes = tokio::fs::read(current_path).await.unwrap();
    assert_eq!(current_bytes, b"second-image");
}
...
#[tokio::test]
#[serial(upload_handler_db)]
async fn test_instant_upload_handler_keeps_existing_media_mime_for_generic_request() {
    init_test_env();
    let pool = create_test_pool().await;
    cleanup_test_data(&pool).await;

    let (user_id, token) = login_and_get_token(&pool, "instant_media_mime").await;
    let app = build_test_app(&pool).await;

    let hash = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
    let source_id = Uuid::new_v4();
    let source_path = format!("{user_id}/{source_id}/source.jpg");
    sqlx::query(
        "INSERT INTO files
         (id, user_id, filename, original_filename, file_path, file_size, mime_type, storage_backend, content_sha256)
         VALUES ($1, $2, 'source.jpg', 'source.jpg', $3, 22, 'image/jpeg', 'local', $4)",
    )
    .bind(source_id)
    .bind(user_id)
    .bind(source_path)
