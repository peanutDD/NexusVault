...
use file_storage_backend::{
    services::storage::{
        is_storage_path_compressed, LocalStorage, StorageBackend, StorageReadStream,
    },
    utils::AppError,
};
use std::path::Path;
...
#[tokio::test]
async fn test_local_storage_compresses_text_files_transparently() {
    init_test_env();
    ...
    let user_id = Uuid::new_v4();
    let file_id = Uuid::new_v4();
    let original = "hello upload optimizer\n".repeat(16_384);

    let file_path = storage
        .save_file(user_id, file_id, "notes.txt", original.as_bytes())
        .await
        .unwrap();

    assert!(
        is_storage_path_compressed(&file_path),
        "text uploads should be stored with a compressed storage marker"
    );

    let stored_size = tokio::fs::metadata(&file_path).await.unwrap().len();
    assert!(
        stored_size < original.len() as u64 / 4,
        "repetitive text should be much smaller on disk"
    );

    let read_back = storage.get_file(&file_path).await.unwrap();
    assert_eq!(read_back, original.as_bytes());
}

#[tokio::test]
async fn test_local_storage_compressed_text_range_reads_original_bytes() {
    init_test_env();

    let temp_dir = tempdir().unwrap();
    let storage = LocalStorage::new(temp_dir.path().to_str().unwrap().to_string());
    let data = "0123456789".repeat(8_192);
    let file_path = storage
        .save_file(Uuid::new_v4(), Uuid::new_v4(), "range.txt", data.as_bytes())
        .await
        .unwrap();
    ...
    match stream {
        StorageReadStream::Memory(mut cursor) => {
            cursor.seek(std::io::SeekFrom::Start(5)).await.unwrap();
            cursor.take(10).read_to_end(&mut buf).await.unwrap();
        }
        other => panic!(
            "compressed text should use memory stream, got {:?}",
            stream_kind(&other)
        ),
    }

    assert_eq!(buf, b"5678901234");
}

fn stream_kind(stream: &StorageReadStream) -> &'static str {
    match stream {
        StorageReadStream::Local(_) => "local",
        StorageReadStream::S3(_) => "s3",
        StorageReadStream::Memory(_) => "memory",
    }
}
