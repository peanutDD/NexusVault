...
impl FileService {
    ...
    pub(crate) async fn ensure_folder_belongs_to_user(
        &self,
        user_id: Uuid,
    ...
    pub async fn enqueue_fulltext_rebuild_for_user(
        &self,
        user_id: Uuid,
        ...
        let file_ids = sqlx::query_scalar::<_, Uuid>(
            "SELECT id FROM files WHERE user_id = $1 AND deleted_at IS NULL ORDER BY created_at ASC",
        )
        .bind(user_id)
        .fetch_all(&self.pool)
        .await
        ...
        for file_id in &file_ids {
            self.enqueue_fulltext_index_task(*file_id, user_id).await?;
        }
