...
use crate::utils::{
    effective_file_mime_type, is_generic_mime_type, normalize_filename_extension, AppError,
};
...
impl FileService {
    ...
    pub async fn instant_upload(
        ...
        let mut mime_type = effective_file_mime_type(&req.filename, Some(&req.mime_type));
        if is_generic_mime_type(&mime_type) && !is_generic_mime_type(&existing.mime_type) {
            mime_type = existing.mime_type.clone();
        }
        let original_filename = normalize_filename_extension(&req.filename, &mime_type);

        self.ensure_can_store_detailed(user_id, &mime_type, req.file_size)
            .await?;
        ...
        let existing_file = self
            .files_repo
            .find_by_name_and_folder(user_id, &original_filename, req.folder_id)
            .await?;
