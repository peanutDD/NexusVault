...
use crate::utils::{
    effective_file_mime_type, is_generic_mime_type, normalize_filename_extension,
    sniff_mime_type_from_bytes, AppError,
};
use sqlx::PgPool;
...
async fn sniff_mime_type_from_path(path: &Path) -> Option<String> {
    use tokio::io::AsyncReadExt;

    let mut file = tokio::fs::File::open(path).await.ok()?;
    let mut buffer = [0_u8; 512];
    let len = file.read(&mut buffer).await.ok()?;
    sniff_mime_type_from_bytes(&buffer[..len]).map(str::to_string)
}

async fn resolve_upload_metadata(
    original_filename: String,
    supplied_mime_type: String,
    source_path: &Path,
) -> (String, String) {
    let mut mime_type = effective_file_mime_type(&original_filename, Some(&supplied_mime_type));
    if is_generic_mime_type(&mime_type) {
        if let Some(sniffed) = sniff_mime_type_from_path(source_path).await {
            mime_type = sniffed;
        }
    }

    let original_filename = normalize_filename_extension(&original_filename, &mime_type);
...
impl FileService {
    ...
    pub async fn create_file(
        ...
        let mut mime_type = effective_file_mime_type(&original_filename, Some(&mime_type));
        if is_generic_mime_type(&mime_type) {
            if let Some(sniffed) = sniff_mime_type_from_bytes(&data) {
    ...
    pub(crate) async fn create_file_from_path(
        ...
        let (original_filename, mime_type) =
            resolve_upload_metadata(original_filename, mime_type, source_path).await;
        ...
        // 检查是否存在同名文件（在同一文件夹下）
        let existing_file = if allow_versioning {
            self.files_repo
        ...
        let file_id = if let Some(existing) = existing_file {
            ...
            let version_file_path = self
                .try_snapshot_file_to_version_path(
                    user_id,
                    existing_file_id,
                    next_version,
                    &existing.file_path,
            ...
            if let Some(version_file_path) = version_file_path {
                let _ = self
                    .file_versions_repo
                    .create_version(
                        existing_file_id,
            ...
            let old_version_paths = self
                ...
                .cleanup_old_versions(
                    existing_file_id,
                    self.config.storage.file_version_keep_count,
                )
                .await?;
