...
impl FileService {
    ...
    pub async fn list_files(
        ...
        let mut responses: Vec<FileResponse> = files.into_iter().map(FileResponse::from).collect();
        self.attach_file_tags(user_id, &mut responses).await?;
        Ok((responses, total, result.next_cursor))
    }

    async fn attach_file_tags(
        &self,
        user_id: Uuid,
        files: &mut [FileResponse],
    ) -> Result<(), AppError> {
        if files.is_empty() {
            return Ok(());
        }
        ...
        >(
            "SELECT fta.file_id, ft.id, ft.name, ft.color, ft.created_at
             FROM file_tag_assignments fta
             JOIN file_tags ft ON ft.id = fta.tag_id AND ft.user_id = fta.user_id
             WHERE fta.user_id = $1 AND fta.file_id = ANY($2)
        ...
        let mut by_file: HashMap<Uuid, Vec<crate::types::tag::FileTagResponse>> = HashMap::new();
        for (file_id, id, name, color, created_at) in rows {
            by_file
                .entry(file_id)
                .or_default()
                .push(crate::types::tag::FileTagResponse {
                    id,
                    name,
                    color,
                    created_at,
        ...
        for file in files {
            file.tags = by_file.remove(&file.id).unwrap_or_default();
        }
        Ok(())
    }
