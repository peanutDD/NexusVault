...
use crate::{
    ...
    models::api_token::{ApiToken, ApiTokenListItem, CreateApiTokenRequest, UpdateApiTokenRequest},
    repositories::{
        api_tokens::{CreateApiTokenRecord, UpdateApiTokenRecord},
        ApiTokensRepo, FoldersRepo,
    },
    utils::AppError,
};
...
impl ApiTokenService {
    ...
    pub async fn update_token(
    ...
    ) -> Result<ApiTokenListItem, AppError> {
        if let Some(root_folder_id) = req.webdav_root_folder_id.flatten() {
            let folders = FoldersRepo::new(&self.pool);
            if !folders.exists(root_folder_id, user_id).await? {
                return Err(AppError::Validation(
                    "WebDAV root folder does not exist".to_string(),
        ...
        let token = repo
            .update_owned(
                ...
                UpdateApiTokenRecord {
                    ...
                    webdav_read_only: req.webdav_read_only,
                    webdav_root_folder_id: req.webdav_root_folder_id,
                },
            )
            .await?
        ...
        Ok(ApiTokenListItem::from(token))
    }
