...
impl ShareService {
    ...
    pub async fn record_share_event(
        ...
        SharesRepo::new(&self.pool)
            .record_event(share.id, share.user_id, event_type, status)
            .await
            .map(|_| {
                let audit = AuditService::new(self.pool.clone());
                let event_type = match event_type {
                    "download" => "share.downloaded",
                    _ => "share.accessed",
                };
                ...
                tokio::spawn(async move {
                    audit
                        .record_lenient(AuditEventInput {
                            ...
                            ip_address: None,
                            user_agent: None,
                            metadata: serde_json::json!({ "event": event_type }),
                        })
                        .await;
                });
            })
    ...
    pub async fn update_share(
        ...
        let hashed_password = if req.clear_password.unwrap_or(false) {
            None
        } else if let Some(password) = req.password.as_ref().filter(|p| !p.trim().is_empty()) {
            Some(hash_password(password)?)
        } else {
            None
        };
        let password_hash = if req.clear_password.unwrap_or(false) {
            Some(None)
        } else {
            hashed_password.as_deref().map(Some)
        };
        let expires_at = calculate_expiration(req.expires_in_days);
        let share = SharesRepo::new(&self.pool)
            .update(
                share_id,
                user_id,
                password_hash,
    ...
    pub async fn delete_share(&self, share_id: Uuid, user_id: Uuid) -> Result<(), AppError> {
        ...
        AuditService::new(self.pool.clone())
            .record(AuditEventInput {
                ...
                ip_address: None,
                user_agent: None,
                metadata: serde_json::json!({ "share_id": share_id }),
            })
            .await?;
