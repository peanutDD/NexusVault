...
impl FileContentExtractor {
    ...
    pub fn extract_text(
        ...
        match mime_type {
            ...
            // 明确的二进制媒体类型不尝试走文本提取，避免把随机字节写进搜索索引。
            ...
            | "application/x-7z-compressed" => Ok(String::new()),
            // PDF
    ...
    fn extract_by_extension(data: &[u8], filename: &str) -> Result<String, AppError> {
        ...
        match ext.as_str() {
            ...
            "jpg" | "jpeg" | "png" | "gif" | "webp" | "heic" | "heif" | "avif" | "mp3" | "wav"
            | "m4a" | "aac" | "mp4" | "mov" | "m4v" | "zip" | "rar" | "7z" | "bin" => {
                Ok(String::new())
            }
...
#[cfg(test)]
mod tests {
    use super::FileContentExtractor;

    #[test]
    fn extract_text_skips_binary_image_payloads() {
        let result = FileContentExtractor::extract_text(
            b"\xff\xd8\xff\xe0binary-jpeg-payload",
            "image/jpeg",
            "photo.jpg",
        )
        .unwrap();
