...
impl Config {
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        let mut builder = config::Config::builder()
            ...
            .set_default("storage.file_version_keep_count", "20")?
            ...
            .set_default("search.ocr_enabled", "true")?
        ...
        let aliases = [
            ...
            ("FILE_VERSION_KEEP_COUNT", "storage.file_version_keep_count"),
        ...
        // 本地存储路径必须与进程 cwd 解耦，避免从仓库根目录启动时把
        // ./uploads 解析到错误位置并触发维护任务误判。
        if config.storage.backend.eq_ignore_ascii_case("local") {
            let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
            let storage_path = if config.storage.path.trim().is_empty() {
                manifest_dir.join("uploads")
            } else {
                let configured = PathBuf::from(config.storage.path.trim());
...
mod tests {
    ...
    #[test]
    #[serial(config_env)]
    fn from_env_resolves_relative_storage_path_against_backend_manifest_dir() {
        let keys = [
            "DATABASE_URL",
            "DATABASE__URL",
            "JWT_SECRET",
        ...
        std::env::set_var(
            "DATABASE_URL",
            "postgres://postgres@localhost:5432/file_storage_test",
        );
        std::env::remove_var("DATABASE__URL");
        std::env::set_var("JWT_SECRET", "test-secret");
        std::env::remove_var("AUTH__JWT_SECRET");
        std::env::set_var("STORAGE_PATH", "./uploads");
        std::env::remove_var("STORAGE__PATH");

        let config = Config::from_env().expect("config should load from default envs");

        let expected = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("uploads")
            .to_string_lossy()
            .to_string();
        assert_eq!(config.storage.path, expected);
    }

    #[test]
    #[serial(config_env)]
    fn from_env_defaults_orphan_cleanup_interval_to_twenty_four_hours() {
        let keys = [
            "DATABASE_URL",
            "DATABASE__URL",
            "JWT_SECRET",
            ...
            "STORAGE__PATH",
            "ORPHAN_CLEANUP_INTERVAL_SECS",
            "TASKS__ORPHAN_CLEANUP_INTERVAL_SECS",
        ];
        let _guard = EnvGuard::new(&keys);
        std::env::set_var(
            "DATABASE_URL",
        ...
        std::env::set_var("JWT_SECRET", "test-secret");
        std::env::remove_var("AUTH__JWT_SECRET");
        std::env::set_var("STORAGE_PATH", std::env::temp_dir());
        std::env::remove_var("STORAGE__PATH");
        std::env::remove_var("ORPHAN_CLEANUP_INTERVAL_SECS");
        std::env::remove_var("TASKS__ORPHAN_CLEANUP_INTERVAL_SECS");

        let config = Config::from_env().expect("config should load from default envs");

        assert_eq!(config.tasks.orphan_cleanup_interval_secs, 24 * 60 * 60);
    }

    #[test]
    #[serial(config_env)]
    fn from_env_without_required_env_reports_database_url_not_missing_storage_path() {
        let keys = [
            "DATABASE_URL",
            "DATABASE__URL",
            "JWT_SECRET",
        ...
        assert!(matches!(
            err,
            ConfigError::MissingEnvVar(name) if name == "DATABASE_URL"
        ));
    }

    #[test]
    #[serial(config_env)]
    fn from_env_without_jwt_reports_jwt_secret_not_missing_field() {
        let keys = [
            "DATABASE_URL",
            "DATABASE__URL",
            "JWT_SECRET",
        ...
        let err = Config::from_env().expect_err("JWT_SECRET should be required");

        assert!(matches!(
            err,
            ConfigError::MissingEnvVar(name) if name == "JWT_SECRET"
        ));
    }

    #[test]
    #[serial(config_env)]
    fn from_env_defaults_ocr_to_enabled_when_not_overridden() {
        let keys = [
            "DATABASE_URL",
            "JWT_SECRET",
            "STORAGE_PATH",
        ...
        std::env::set_var("JWT_SECRET", "test-secret");
        std::env::set_var("STORAGE_PATH", std::env::temp_dir());
        std::env::set_var("GITHUB_CLIENT_ID", "");
        std::env::set_var("GITHUB_CLIENT_SECRET", "");
        std::env::set_var("GITHUB_OAUTH_REDIRECT_URI", "");
        std::env::set_var("GOOGLE_CLIENT_ID", "");
        std::env::set_var("GOOGLE_CLIENT_SECRET", "");
        std::env::set_var("GOOGLE_OAUTH_REDIRECT_URI", "");
        std::env::remove_var("OCR_ENABLED");
        std::env::remove_var("FULLTEXT_SEARCH_ENABLED");
        std::env::remove_var("SEARCH_INDEX_PATH");
        ...
        assert!(config.search.ocr_enabled);
    }

    #[test]
    #[serial(config_env)]
    fn from_env_defaults_oauth_when_not_configured() {
        let keys = [
            "DATABASE_URL",
            "JWT_SECRET",
            "STORAGE_PATH",
            ...
            "OAUTH__GITHUB_OAUTH_REDIRECT_URI",
            "OAUTH__GOOGLE_CLIENT_ID",
            "OAUTH__GOOGLE_CLIENT_SECRET",
            "OAUTH__GOOGLE_OAUTH_REDIRECT_URI",
        ];
        let saved: Vec<(String, Option<OsString>)> = keys
            .iter()
            .map(|key| ((*key).to_string(), std::env::var_os(key)))
        ...
        let config = Config::from_env().expect("oauth should be optional");
        ...
        assert!(config.oauth.github_client_id.is_none());
