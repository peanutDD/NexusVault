...
fn beijing_log_offset() -> UtcOffset {
    UtcOffset::from_hms(8, 0, 0).expect("Beijing log offset is a valid UTC offset")
}

#[cfg(test)]
fn beijing_log_offset_seconds() -> i32 {
    beijing_log_offset().whole_seconds()
}

fn beijing_log_timer() -> OffsetTime<Rfc3339> {
    OffsetTime::new(beijing_log_offset(), Rfc3339)
}
...
mod tests {
    ...
    #[test]
    fn backend_log_timer_uses_beijing_offset() {
        assert_eq!(beijing_log_offset_seconds(), 8 * 60 * 60);
    }

    #[test]
    fn backend_log_timestamp_formats_as_beijing_time() {
        let utc_timestamp = time::Date::from_calendar_date(2026, time::Month::May, 18)
            .unwrap()
            .with_hms_nano(6, 49, 33, 151_829_000)
            .unwrap()
            .assume_utc();

        let formatted = utc_timestamp
            .to_offset(beijing_log_offset())
            .format(&Rfc3339)
            .unwrap();

        assert_eq!(formatted, "2026-05-18T14:49:33.151829+08:00");
    }
}
