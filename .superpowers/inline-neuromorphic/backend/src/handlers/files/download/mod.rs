...
use crate::services::storage::is_storage_path_compressed;
...
async fn file_get_or_head_response(
    ...
    if method != Method::HEAD {
        if let Err(err) = state.file_service.mark_file_opened(file_id, user_id).await {
            tracing::warn!(
                file_id = %file_id,
                user_id = %user_id,
                error = ?err,
                "failed to update recent file access timestamp"
            );
        }
    ...
    if state.config.storage.download_mode != "proxy" && !is_storage_path_compressed(&file.file_path)
    {
