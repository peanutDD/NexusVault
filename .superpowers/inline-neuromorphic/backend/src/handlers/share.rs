...
use crate::models::share::{
    AccessShareRequest, BatchShareRequest, CreateShareRequest, UpdateShareRequest,
};
use crate::repositories::SharesRepo;
...
pub async fn list_shares_handler(
    ...
    let base_url = get_frontend_url();
    let shares = SharesRepo::new(&state.pool)
        .list_managed_by_user(user_id, &base_url)
        .await?;
    Ok(json_response(json!({ "shares": shares })))
}
...
pub async fn update_share_handler(
    ...
    axum::Json(req): axum::Json<UpdateShareRequest>,
) -> Result<Response, AppError> {
    let share_service = ShareService::from_state(&state);
    let share = share_service.update_share(share_id, user_id, req).await?;
    Ok(json_response(json!({ "share": {
        "id": share.id,
        "expires_at": share.expires_at,
        "max_downloads": share.max_downloads,
...
pub async fn list_share_events_handler(
    State(state): State<AppState>,
    AuthenticatedUser(user_id): AuthenticatedUser,
    Path(share_id): Path<Uuid>,
) -> Result<Response, AppError> {
    let events = SharesRepo::new(&state.pool)
        .list_events(share_id, user_id)
        .await?;
...
pub async fn access_share_handler(
    ...
    share_service
        .record_share_event(&share, "access", 200)
        .await?;
...
pub async fn download_shared_file_handler(
    ...
    share_service
        .record_share_event(&share, "download", 200)
        .await?;
