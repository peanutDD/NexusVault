...
function fileRequestReviewActionPillToneClass(
  tone: "preview" | "open" | "approve" | "reject",
) {
...
const shareManagedRowBaseClass =
  "share-managed-row neu-raised-sm shareCenterNeuRaisedSurface shareCenterNeuDataRow grid w-full min-w-0 gap-[clamp(0.58rem,1.35vw,0.82rem)] rounded-[clamp(0.78rem,1.8vw,0.95rem)] border-0 p-[clamp(0.78rem,1.8vw,1rem)] transition-[transform,box-shadow]";
...
function shareStatusTone(status: string) {
  ...
  if (
    ["closed", "revoked", "expired", "inactive"].some((value) =>
  ...
  if (
    ["pending", "limited", "queued", "waiting"].some((value) =>
      normalized.includes(value),
    )
  ) {
    return "yellow";
...
function PublishFolderSelector({
  ...
  const currentLabel = folderLocationLabel(
    effectiveFolderId,
    effectiveFolderName,
  );
  ...
  return (
    <div className="grid gap-[clamp(0.35rem,0.8vw,0.45rem)]">
      <button
        ...
        className={cn(
          fileRequestInboxFlatButtonClass,
          ...
          <div className="mb-[clamp(0.35rem,0.8vw,0.45rem)] flex flex-wrap items-center gap-[clamp(0.3rem,0.7vw,0.4rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)]">
            {path.map((item, index) => (
              <button
                ...
                className="neu-raised-sm rounded-[clamp(0.4rem,0.9vw,0.5rem)] px-[clamp(0.35rem,0.8vw,0.45rem)] py-[clamp(0.2rem,0.45vw,0.25rem)] transition-[box-shadow,color] active:shadow-[var(--neu-pressed-shadow)]"
              >
                {item.name}
              </button>
            ))}
          </div>
          <div className="grid neu-pressed p-2 max-h-[clamp(8rem,22vw,12rem)] gap-[clamp(0.25rem,0.6vw,0.35rem)] overflow-auto">
            <button
              type="button"
              ...
              className={cn(
                fileRequestInboxFlatButtonClass,
                "justify-start px-[clamp(0.5rem,1.1vw,0.625rem)] py-[clamp(0.3rem,0.8vw,0.4rem)]",
              )}
            >
              <Home className="h-[clamp(0.72rem,1.6vw,0.9rem)] w-[clamp(0.72rem,1.6vw,0.9rem)]" />
              根目录
              {!effectiveFolderId && (
                <Check className="ml-auto h-[clamp(0.72rem,1.6vw,0.9rem)] w-[clamp(0.72rem,1.6vw,0.9rem)]" />
              )}
            </button>
            {loading && (
              <p className="px-[clamp(0.5rem,1.1vw,0.625rem)] py-[clamp(0.3rem,0.8vw,0.4rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)]">
                加载文件夹中...
              </p>
            )}
            {error && (
              <p className="px-[clamp(0.5rem,1.1vw,0.625rem)] py-[clamp(0.3rem,0.8vw,0.4rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-form-error)]">
                {error}
              </p>
            )}
            {folders.map((folder) => (
              <div
                key={folder.id}
                className="grid grid-cols-[minmax(0,1fr)_auto] gap-[clamp(0.25rem,0.6vw,0.35rem)]"
              >
                <button
                  type="button"
                ...
                <button
                  ...
                  className={cn(
                    fileRequestInboxFlatButtonClass,
                    "px-[clamp(0.45rem,1vw,0.55rem)] py-[clamp(0.3rem,0.8vw,0.4rem)]",
                  )}
                >
...
export default function Shares() {
  ...
  const [oneTimeRequestUrl, setOneTimeRequestUrl] = useState<string | null>(
    null,
  );
  ...
  const showUploads = useCallback(async (request: FileRequestLink) => {
    try {
      ...
      requestDetailRef.current?.scrollIntoView?.({
        behavior: "smooth",
        block: "start",
      });
    } catch (err) {
  ...
  return (
    ...
    >
      <div
        ...
        className="neu-inset settings-neu-inset-panel shareCenterCodepenFrame mx-auto w-full max-w-[var(--app-shell-max-width)] overflow-x-hidden rounded-[clamp(1.25rem,3vw,1.5rem)] border-0 p-[clamp(0.78rem,1.8vw,1rem)] text-[length:var(--settings-text-md)]"
      >
        <div
          data-testid="share-center-shell"
          className="neu-raised settings-neu-raised-card shareCenterCodepenShell mb-[clamp(1rem,2.25vw,1.25rem)] max-w-full overflow-hidden rounded-[clamp(1rem,2.4vw,1.25rem)] border-0 p-[clamp(0.78rem,1.8vw,1rem)]"
        >
          <div
            ...
            <div className="min-w-0">
              <button
                ...
                className={settingsSecondaryButtonClass(
                  "inline-flex items-center px-[clamp(0.585rem,1.35vw,0.75rem)] py-[clamp(0.39rem,0.9vw,0.5rem)] text-[length:var(--settings-text-xs)]",
                )}
              >
        ...
        {tab === "shares" ? (
          ...
          >
            <section
              ...
              className="neu-raised shareCenterNeuRaisedPanel rounded-[clamp(1rem,2.4vw,1.25rem)] border-0 p-[clamp(0.78rem,1.8vw,1rem)]"
            >
              <div className="mb-[clamp(0.58rem,1.35vw,0.75rem)] flex items-center justify-between gap-[clamp(0.58rem,1.35vw,0.75rem)]">
                ...
                <span className="neu-raised-sm shareCenterFlatBadge rounded-full border-0 px-[clamp(0.5rem,1.1vw,0.625rem)] py-[clamp(0.2rem,0.45vw,0.25rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)]">
                  links
                </span>
              </div>
              <div
                ...
                className={settingsPanelClass(
                  managementListScrollClass(rows.length),
                )}
              >
                {rows.map((share) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isActive && "neu-inset shareCenterNeuPressedSurface",
                      )}
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-[clamp(0.195rem,0.45vw,0.25rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)]">
                          access {share.access_count} · downloads{" "}
                          {share.download_count} · {shareLimitText(share)}
                        </p>
                        <div
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            copy(
                              share.url || absoluteShareUrl(share.share_token),
                            )
                          }
                        >
                        ...
                        <button
                          ...
                          aria-label={`Revoke ${share.filename}`}
                        >
                ...
                {!rows.length && (
                  <p className="p-[clamp(1rem,2.25vw,1.25rem)] text-[var(--settings-subtitle)]">
                    还没有分享链接。
                  </p>
                )}
              </div>
            ...
            <aside
              ...
              className="neu-raised shareCenterNeuRaisedPanel shareCenterShareDetailPanel min-h-[clamp(12rem,30vw,18rem)] rounded-[clamp(0.7rem,1.6vw,0.75rem)] border-0 p-[clamp(0.78rem,1.8vw,1rem)] transition-[box-shadow] duration-150"
            >
              {activeShareEvents ? (
                <>
                  <div
                  ...
                  >
                    <div
                      data-testid="share-events-title-block"
                    ...
                    <button
                      ...
                      className={settingsSecondaryButtonClass(
                        "shareCenterClosePillButton shareCenterNeuActionButton ml-auto shrink-0 self-center rounded-full px-[clamp(0.7em,1.8vw,1em)] py-[clamp(0.35em,0.9vw,0.5em)] text-[length:var(--settings-text-xs)]",
                      )}
                  ...
                  <div
                    ...
                    className="share-events-list neu-inset shareCenterNeuDataList shareCenterNeuInsetSurface max-h-[min(42vh,24rem)] overflow-auto rounded-[clamp(0.72rem,1.6vw,0.85rem)] border-0 p-[clamp(0.58rem,1.35vw,0.75rem)]"
                  >
                    {activeShareEvents.events.map((event) => (
                      <div
                        key={event.id}
                        data-testid={`share-event-row-${event.id}`}
                        className="neu-raised-sm shareCenterNeuDataRow shareCenterNeuRaisedSurface grid gap-[clamp(0.25rem,0.7vw,0.375rem)] rounded-[clamp(0.62rem,1.4vw,0.75rem)] border-0 p-[clamp(0.48rem,1.1vw,0.62rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)] sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center"
                      >
                ...
                <div
                  ...
                  className="neu-inset shareCenterNeuInsetSurface flex min-h-[clamp(9rem,24vw,14rem)] flex-col items-center justify-center rounded-[clamp(0.7rem,1.6vw,0.75rem)] border-0 p-[clamp(0.78rem,1.8vw,1rem)] text-center text-[var(--settings-subtitle)]"
                >
                  <p className="font-semibold text-[var(--settings-title)]">
                    No share selected
                  </p>
          ...
          >
            ...
            >
              ...
              {oneTimeRequestUrl && (
                <div
                  data-testid="file-request-secret-panel"
                  className={settingsPanelClass(
                    "mb-[clamp(0.78rem,1.8vw,1rem)]",
                  )}
                >
                  <div className="flex flex-col gap-[clamp(0.58rem,1.35vw,0.75rem)] sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className={shareCenterInlineActionGroupClass}>
                      <button
                        ...
                        className={settingsSecondaryButtonClass(
                          `${fileRequestFluidActionClass} text-[length:var(--settings-text-xs)]`,
                        )}
              ...
              <div
                ...
                className={settingsPanelClass(
                  managementListScrollClass(requests.length),
                )}
              >
                {requests.map((request) => (
                  ...
                  >
                    <div className="min-w-0">
                      <p className="truncate font-semibold text-[var(--settings-title)]">
                        {request.title}
                      </p>
                      <p className="mt-[clamp(0.195rem,0.45vw,0.25rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)]">
                        {request.revoked_at ? "revoked" : "active"} · uploads{" "}
                        {request.upload_count}
                        {request.max_file_size
                          ? ` · ${formatFileSize(request.max_file_size)}`
                          : ""}
                      </p>
                      {!request.public_url && (
                        <p className="mt-[clamp(0.195rem,0.45vw,0.25rem)] text-[length:var(--settings-text-xs)] text-[var(--settings-subtitle)]">
                ...
                <>
                  ...
                  >
                    ...
                    <button
                      ...
                      className={cn(
                        fileRequestInboxFlatButtonClass,
                        "ml-auto shrink-0 self-center px-[clamp(0.5rem,1.1vw,0.625rem)] py-[clamp(0.3rem,0.8vw,0.4rem)]",
                      )}
                  ...
                  >
                    {activeUploads.submissions.map((submission) => {
                      const submissionTargetId =
                        submission.request_folder_id ??
                        activeUploads.request.folder_id;
                      const submissionTargetName =
                        submission.request_folder_name ??
                        activeUploads.request.folder_name;
                      const submissionTargetLabel = folderLocationLabel(
                        submissionTargetId,
                        ...
                        >
                          <div className="grid gap-[clamp(0.28rem,0.65vw,0.36rem)]">
                            ...
                            <div className="flex flex-col gap-[clamp(0.35rem,0.8vw,0.45rem)] sm:flex-row sm:items-end sm:justify-between">
                              ...
                              <p className="fileRequestInboxMutedText text-[length:var(--settings-text-xs)] font-medium">
                                {submission.file_count} file
                                {submission.file_count === 1 ? "" : "s"}
                              </p>
                            </div>
                          </div>
                          <div
                          ...
                          >
                            {submission.uploads.map((upload) => {
                              ...
                              const UploadStatusIcon = shareStatusIcon(
                                upload.status,
                              );
                              return (
                                <article
                                ...
                                >
                                    ...
                                    >
                                      <span
                                        ...
                                        className={cn(
                                          shareBadgeClass,
                                          shareBadgeToneClass(
                                            shareStatusTone(upload.status),
                                          ),
                                        )}
                                      >
                                        <UploadStatusIcon
                                          className={shareBadgeIconClass}
                                        />
                                    ...
                                    >
                                      <div className="grid gap-[clamp(0.28rem,0.65vw,0.38rem)]">
                                        ...
                                        <input
                                          ...
                                          disabled={!isPending}
                                          onChange={(event) =>
                                            updateReviewDraft(upload, {
                                              filename: event.target.value,
                                            })
                                          }
                                          className={
                                            fileRequestInboxFlatInputClass
                                          }
                                        />
                                      </div>
                                      <div className="grid gap-[clamp(0.28rem,0.65vw,0.38rem)]">
                                        ...
                                        {isPending ? (
                                          ...
                                          <p
                                            className={cn(
                                              fileRequestInboxFlatFieldClass,
                                  ...
                                  >
                                    <a
                                      href={
                                        isApproved
                                          ? fileRequestService.previewApprovedFileUrl(
                                              upload.file_id,
                                            )
                                          : fileRequestService.previewUploadUrl(
                                              upload.id,
                                            )
                                      }
                                      target="_blank"
                                      ...
                                      className={cn(
                                        ...
                                        fileRequestReviewActionPillToneClass(
                                          "preview",
                                        ),
                                      )}
                                    >
                    ...
                    {activeUploads.nextCursor && (
                      <button
                        ...
                        className={cn(
                          fileRequestInboxFlatButtonClass,
                          "mt-[clamp(0.58rem,1.35vw,0.75rem)] w-full",
                        )}
                      >
