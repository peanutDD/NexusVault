...
describe("light theme tokens", () => {
  ...
  it("keeps the auth page flat while scoping Shape Wave tokens to the decorative background", () => {
    const tokens = readCssCustomProperties(LIGHT_NEU_SELECTOR);
  ...
  it("uses pure primary action fills and keeps AJgeEd fireworks transparent over the light theme", () => {
    const tokens = readCssCustomProperties(LIGHT_NEU_SELECTOR);
    ...
    expect(tokens.has("--filelist-fireworks-bg")).toBe(false);
