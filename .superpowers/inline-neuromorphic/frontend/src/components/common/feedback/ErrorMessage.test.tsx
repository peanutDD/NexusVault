...
describe("ErrorMessage", () => {
  it("keeps alert behavior while exposing hooks for the neuromorphic alert treatment", () => {
    ...
    expect(alert).not.toHaveClass("neu-semantic-raised");
  ...
  it("uses the shared raised primitive without decorative alert layers", () => {
    const source = readFileSync("src/components/common/feedback/ErrorMessage.tsx", "utf8");

    expect(source).toContain('"neu-raised"');
    expect(source).not.toContain('"neu-semantic-raised"');
