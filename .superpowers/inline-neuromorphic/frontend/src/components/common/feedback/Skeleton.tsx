...
export function FileListSkeleton({ count = 5 }: { count?: number }) {
  return (
    <div
      className="neu-inset min-w-[var(--app-filelist-skeleton-min-width)] overflow-hidden rounded-[clamp(0.4rem,1vw,0.5rem)]"
      data-oid="rzzj7yc"
    >
      <table className="w-full table-fixed border-collapse" data-oid="by7hc9y">
        <thead data-oid="b__upe0">
          <tr
            className="border-b border-[var(--skeleton-border)]"
            data-oid="4yuyl-a"
          >
...
function FileCardSkeletonItem() {
  return (
    <div
      className="neu-inset rounded-[clamp(0.3rem,0.8vw,0.375rem)] p-[clamp(0.585rem,1.35vw,0.75rem)]"
      data-oid="3yxzj:g"
    >
      {/* 缩略图占位 */}
      <Skeleton
