...
describe("ShareDialog", () => {
  ...
  it("marks the single share modal internals for the neuromorphic CodePen treatment", () => {
    ...
    expect(screen.getByRole("dialog", { name: "分享文件" })).not.toHaveClass(
      "modal-dialog-tech",
    );
    expect(
  ...
  it("maps single share dialog hooks to raised and inset neuromorphic primitives", () => {
    ...
    expect(css).toContain('.singleShareDialogPanel');
    expect(css).toContain("background: var(--neu-inset-bg)");
    expect(css).toContain("box-shadow: var(--neu-inset-shadow)");
    expect(css).toContain('.singleShareDialogField');
    expect(css).toContain('.singleShareDialogAction');
    ...
    expect(css).not.toContain('.singleShareDialogShell .modal-dialog-tech-grid');
    expect(css).toContain('.singleShareDialogPrimary');
    expect(css).not.toMatch(/linear-gradient|radial-gradient|neuromorphic-style/);
  });
