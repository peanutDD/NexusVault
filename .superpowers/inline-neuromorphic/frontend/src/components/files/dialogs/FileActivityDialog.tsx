        ...
        {!isInitialActivityLoading && !activity.isError && events.length > 0 && (
          ...
          >
            {events.map((event) => (
              <div
                key={event.id}
                className="fileActionDialogInsetRow mb-[clamp(0.585rem,1.35vw,0.75rem)] rounded-[clamp(0.6rem,1.35vw,0.75rem)] border-0 p-[clamp(0.78rem,1.8vw,1rem)] last:mb-0"
              >
            ...
            {nextCursor && (
              <div className="flex justify-center py-[clamp(0.39rem,0.9vw,0.5rem)]">
                <button
                  ...
                  className="singleShareDialogAction neu-raised-sm rounded-[clamp(0.4rem,1vw,0.5rem)] border-0 px-[clamp(0.58rem,1.35vw,0.75rem)] py-[clamp(0.3rem,0.8vw,0.4rem)] text-[var(--dialog-action-text)] transition-[box-shadow,color] active:shadow-[var(--neu-pressed-shadow)] disabled:opacity-60"
                >
