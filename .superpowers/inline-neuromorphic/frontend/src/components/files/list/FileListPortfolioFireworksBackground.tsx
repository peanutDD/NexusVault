...
function drawStillBurst(ctx: CanvasRenderingContext2D, width: number, height: number, hue: number) {
  const x = width / 2;
  const y = height * 0.28;
  const rays = 14;
  ...
  for (let index = 0; index < rays; index += 1) {
    ...
    ctx.strokeStyle = `hsla(${hue + index * 8}, 100%, 65%, 0.72)`;
    ctx.stroke();
  }
}
...
export default function FileListPortfolioFireworksBackground() {
  ...
  useEffect(() => {
    ...
    const renderStill = () => {
      ...
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      drawStillBurst(ctx, width, height, hue);
    };
    ...
    const loop = () => {
      ...
      // Erase prior canvas strokes only; the page theme remains the visible background.
