...
export function createMarkdownComponents(
  ...
  return {
    ...
    code: ({ inline, className, children, ...props }) => {
      ...
      return isInlineCode ? (
        <code
          className="neu-inset rounded border-0 px-[clamp(0.195rem,0.45vw,0.25rem)] py-[clamp(0.0975rem,0.3vw,0.125rem)] text-[length:var(--font-size-ui-3xs)] font-mono text-[var(--preview-markdown-code-inline-text)]"
          {...props}
        >
          {children as ReactNode}
        </code>
    ...
    pre: ({ children, ...props }) => {
      ...
      return (
        <pre
          className="neu-inset previewMarkdownPre mb-[clamp(0.585rem,1.35vw,0.75rem)] overflow-auto rounded-[clamp(0.3rem,0.8vw,0.375rem)] border-0 p-[clamp(0.585rem,1.35vw,0.75rem)] text-[clamp(0.68rem,1.6vw,0.75rem)] font-mono text-[var(--preview-markdown-pre-text)]"
          {...props}
        >
          {children as ReactNode}
        </pre>
    ...
    table: ({ ...props }) => (
      <div className="neu-inset previewMarkdownTableShell mb-[clamp(0.585rem,1.35vw,0.75rem)] overflow-auto rounded-[clamp(0.3rem,0.8vw,0.375rem)] border-0">
        <table className="min-w-full text-[clamp(0.7rem,1.45vw,0.74rem)] text-[var(--preview-markdown-table-text)]" {...props} />
      </div>
    ),
    th: ({ ...props }) => (
      <th className="neu-raised-sm border-b px-[clamp(0.39rem,0.9vw,0.5rem)] py-[clamp(0.195rem,0.45vw,0.25rem)] text-left font-semibold border-transparent text-[var(--preview-markdown-table-head-text)]" {...props} />
    ),
    td: ({ ...props }) => (
