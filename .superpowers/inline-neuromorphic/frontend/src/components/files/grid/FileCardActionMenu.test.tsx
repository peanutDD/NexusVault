...
describe("card action menus", () => {
  ...
  it("marks file card menu panel and items for the neuromorphic action-menu treatment", () => {
    ...
    expect(menu?.closest(".fileCardSurface")).toHaveClass("fileCardMenuOpen");
  ...
  it("maps neuromorphic card menus to raised panels and inset hover list items", () => {
    ...
    const fileSource = readFileSync(resolve(__dirname, "FileCard.tsx"), "utf8");
    const folderSource = readFileSync(resolve(__dirname, "FolderCard.tsx"), "utf8");

    expect(css).toContain(".fileCardSurface");
    expect(css).toContain(".fileCardMenuOpen");
    expect(css).toContain(".fileCardActionMenu");
    expect(css).toContain("--file-card-action-menu-icon-size");
    expect(css).toContain(
      "grid-template-columns: var(--file-card-action-menu-icon-size) minmax(0, 1fr);",
    );
    expect(css).toContain(".fileCardActionMenuItem:hover");
    expect(fileSource).not.toContain("text-[clamp(8px");
    expect(folderSource).not.toContain("clamp(90px");
    expect(folderSource).not.toContain("text-[clamp(8px");
