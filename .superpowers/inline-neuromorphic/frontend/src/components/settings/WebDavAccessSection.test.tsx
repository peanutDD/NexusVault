...
describe("WebDavAccessSection", () => {
  ...
  it("renders the Basic Auth and Copy URL controls as matching primary neuromorphic pills", () => {
    ...
    [chip, copyButton, copyUsernameButton].forEach((control) => {
      ...
      expect(control).toHaveClass("neu-raised-green");
      ...
      expect(control).not.toHaveClass("neu-raised-sm");
      expect(control).not.toHaveClass("[background:var(--settings-action-bg)]");
    });
  ...
  it("keeps Copy username on the shared raised primary pill primitive", () => {
    render(<WebDavAccessSection />);

    const copyUsernameButton = screen.getByRole("button", { name: /Copy username/i });

    expect(copyUsernameButton).toHaveClass("settings-neu-primary-pill");
    expect(copyUsernameButton).toHaveClass("neu-raised-green");
    expect(copyUsernameButton).not.toHaveClass("neu-raised-sm");
    expect(copyUsernameButton).not.toHaveClass("[background:var(--settings-action-bg)]");
  });
