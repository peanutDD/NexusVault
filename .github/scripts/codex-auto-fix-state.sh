...
apply_plan() {
  ...
  case "$action" in
    ...
    needs_human)
      add_label "$current"
      add_label "gemini-review-needs-human"
      gh pr comment "${PR_NUMBER:?}" --body "🤖 **Codex 未能清理当前 Gemini Review 的 Medium/Medium+/High/Critical 问题。** 上方 Codex 分析评论应已按 Gemini 问题一一对应列出状态、已解决项与未解决原因；本轮不会误判为可合并，请人工处理或重跑。"
      ;;
    advance|advance_with_pending)
      ...
      if [[ "$action" == "advance_with_pending" ]]; then
        gh pr comment "${PR_NUMBER:?}" --body "🤖 **Codex 已推送部分修复，并请求下一轮 Gemini Review。** 当前仍有 Medium/Medium+/High/Critical 未自动修复说明，下一轮后仍存在则需要人工决策。"
      else
    ...
    complete)
      ...
      gh pr comment "${PR_NUMBER:?}" --body "🤖 **Codex/Gemini 自动 Review 闭环已完成 ${max_rounds} 轮，当前没有 Medium/Medium+/High/Critical 未处理项。** 请人工做最终 diff Review 后决定是否合并。"
      ;;
    complete_with_pending)
      add_label "gemini-review-round-max"
      add_label "gemini-review-needs-human"
      gh pr comment "${PR_NUMBER:?}" --body "🤖 **Codex/Gemini 自动 Review 已达到 ${max_rounds} 轮，但仍有 Medium/Medium+/High/Critical 未自动修复说明。** 请人工处理或明确接受这些 pending 后再合并。"
      ;;
  esac
}
